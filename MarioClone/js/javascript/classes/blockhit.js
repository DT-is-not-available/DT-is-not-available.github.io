	...
	draw() {
		if (this.timer >= -10) {
			if (!(typeof(level.temptiles[this.x+","+this.y]) == 'undefined')) {
				...
				canvas.drawImage(img_tileset, tile_defs[level.temptiles[this.x+","+this.y]].tileX*16, tile_defs[level.temptiles[this.x+","+this.y]].tileY*16, 16, 16, Math.round(this.x*16-camera_x), Math.round(this.y*16+(10-Math.abs(this.timer))*this.gravity-camera_y), 16, 16);
